...
function curl ($url, $post = 0, $httpheader = 0, $proxy = 0, $uagent = 0){
    $ch = curl_init();
...
function getRandName(){
    $curl = curl("https://uinames.com/api/?region=indonesia")[1];
    $curl = json_decode($curl, true);
    return array(
        "fullname" => $curl['name']." ".$curl['surname'],
        "password" => $curl['name'].$curl['surname'].generateRandomString(3),
        "mail" => strtolower($curl['name']).strtolower($curl['surname']).generateRandomString(3)."@gmail.com"
        );
}
?>
